...
export function BlogShelf({ articles }) {
  ...
  return <div className="blog-studio wrap">
    <section className="bs-hero"><div><p className="bs-eyebrow">// NOTES FROM BUILDING</p><h1>Behind the code.<br /><span>Inside the decisions.</span><i aria-hidden="true">✦</i></h1><p className="bs-intro">System design, LLD, and patterns put into practice. Notes from my projects and my reading of Alex Xu and Alexander Shvets — the trade-offs, the failures, and the decisions behind the code.</p></div>
      <div className="bs-topics" aria-label="Filter articles by topic">{blogTopics.map((t, i) => <button key={t} className="bs-orb" data-topic={t} style={{"--i":i}} aria-pressed={topic === t} onClick={() => chooseTopic(topic === t ? "All" : t, true)}><strong>{t}</strong></button>)}<p>Explore a topic ↗</p></div>
    </section>
